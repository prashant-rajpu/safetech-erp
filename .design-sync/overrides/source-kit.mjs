// forked from design-sync lib/source-kit.mjs — this repo's components are all
// `export default function Name() {}` (an application, not a library with
// named exports). The stock synth-entry writer emits `export * from "path"`
// per file, but `export *` never re-exports a module's `default` binding —
// so esbuild's IIFE bundling produced zero properties on window.SafetechDS
// ([BUNDLE_EXPORT] 7/7 not a component). This fork additionally emits a named
// re-export (`export { default as Name } from "path"`) for every file where
// ts-morph can recover the default export's declared name, using the exact
// same recovery logic deriveComponentsFromSrc already uses for discovery —
// just applied at entry-synthesis time instead of only at name-scan time.
...
// Shared ts-morph project for both default-name recovery (entry synthesis)
// and full discovery (deriveComponentsFromSrc) — one parse pass per file.
...
// `export default function Button() {}` is keyed as 'default' in
// getExportedDeclarations() — recover the declared name from the node.
function recoverDefaultExportName(project, path) {
  ...
  if (!real || !/^[A-Z][A-Za-z0-9]*$/.test(real)) return null;
  if (!decls.some((d) => Node.isVariableDeclaration(d) || Node.isFunctionDeclaration(d) || Node.isClassDeclaration(d))) return null;
  return real;
}
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const entryProject = makeProject();
    let defaultRecovered = 0;
    const lines = comps.flatMap((p) => {
      const out = [`export * from ${JSON.stringify(p)};`];
      const name = recoverDefaultExportName(entryProject, p);
      if (name) {
        out.push(`export { default as ${name} } from ${JSON.stringify(p)};`);
        defaultRecovered++;
      }
      return out;
    });
    ...
    console.error(
      `[NO_DIST] no built entry — synthesizing from ${comps.length} src files ` +
        `(${defaultRecovered} default-export name(s) recovered; run the package's build for best results)`,
    );
  }
