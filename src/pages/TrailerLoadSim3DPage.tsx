...
export default function TrailerLoadSim3DPage() {
  ...
  if (loading) return <div className="p-6 text-red-500 font-semibold flex items-center justify-center min-h-[300px] animate-pulse">Loading trailer loading simulation…</div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between pb-4 border-b border-slate-200 dark:border-white/5">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight text-neutral-900 dark:text-white uppercase">
            Trailer Loading <span className="text-red-500 font-light">3D Simulation</span>
          </h2>
      ...
      <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-2 text-[11px] text-amber-500 font-semibold">
        ⚠ Illustrative live check only — nothing here is saved. Checks bed-boundary fit and total weight vs. capacity; does not detect elements overlapping each other.
      </div>
      ...
      {trailers.length === 0 ? (
        ...
        <>
          <div className={`glass-panel rounded-2xl p-4 border ${withinCapacity && !anyOutOfBounds ? 'border-emerald-500/30' : 'border-red-500/30'}`}>
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex gap-2">
                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${withinCapacity ? 'bg-emerald-500/10 text-emerald-500 border border-emerald-500/20' : 'bg-red-500/10 text-red-500 border border-red-500/20'}`}>
                  {totalWeight.toFixed(1)}T / {capacity}T {withinCapacity ? '✓' : '✗ Over Capacity'}
                </span>
                {anyOutOfBounds && (
                  <span className="px-3 py-1 rounded-full text-[10px] font-black uppercase bg-red-500/10 text-red-500 border border-red-500/20">
                    ✗ Element(s) Out of Bed Bounds
                  </span>
                )}
              </div>
            </div>
